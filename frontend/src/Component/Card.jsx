...
function Card({title,landMark,image1,image2,image3,rent,city,id,ratings,isBooked,host}){
  ...
  return(
    <div className="w-[330px] max-w-[85%] h-[460px] flex items-start justify-start flex-col rounded-lg cursor-pointer relative z-[10] " onClick={()=>!isBooked?handleClick():null}>
     {isBooked && <div className="text-[green] bg-white rounded-lg absolute flex items-center justify-center 
      right-1 top-1 gap-[5px] p-[5px]"><GiConfirmed  className="w-[20px] h-[20px] text-[green]"/>Booked</div>}
...
<span className="flex items-center justify-center gap-[5px]"><FaStar className="text-[#eb6262]" />{ratings}</span></div>
